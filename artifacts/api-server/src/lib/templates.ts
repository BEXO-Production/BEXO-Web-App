...
export const getStaffInviteEmail = (opts: {
  ...
  return wrapHtml({
    ...
    bodyHtml: `
      ...
      ${bodyParagraph(
        `Sign in anytime at <a href="${escapeHtml(opts.loginUrl)}" style="color:${INK};font-weight:600;">${escapeHtml(opts.loginUrl.replace(/^https?:\/\//, ""))}</a> with the email and temporary password. You will be asked to choose a new password after sign-in.`,
      )}
      ${bodyParagraph(
...
export const getStaffPasswordResetEmail = (opts: {
  userName: string;
  email: string;
  temporaryPassword: string;
  resetUrl: string;
  ...
  wrapHtml({
    title: "Reset your BEXO Admin password",
    preheader: `Temporary password inside — reset link expires ${opts.expiresLabel}.`,
    eyebrow: "Password reset",
    headline: "Reset your BEXO Admin password.",
    accent: "amber",
    ctaLabel: "Choose a new password",
    ctaUrl: opts.resetUrl,
    footnote:
      "BEXO Admin · Ace Digital · If you did not request this, contact a super admin immediately.",
    bodyHtml: `
      ${greeting(opts.userName)}
      ${bodyParagraph(
        `A super admin reset your <strong style="color:${INK};">BEXO Admin</strong> password for <strong style="color:${INK};">${escapeHtml(opts.email)}</strong>.`,
      )}
      ${highlightCard(
        "Temporary password",
        `<span style="font-family:ui-monospace,Menlo,Consolas,monospace;font-size:16px;letter-spacing:0.04em;">${escapeHtml(opts.temporaryPassword)}</span>`,
      ...
      ${highlightCard("Reset link expires", escapeHtml(opts.expiresLabel), "#FEE2E2")}
      ${bodyParagraph(
        `Open the button above to choose a new password, or sign in at <a href="${escapeHtml(opts.loginUrl)}" style="color:${INK};font-weight:600;">${escapeHtml(opts.loginUrl.replace(/^https?:\/\//, ""))}</a> with the temporary password — you will be prompted to change it immediately.`,
      )}
      ${featurePills(["Temp password", "24h reset link", "Forced change"])}
    `,
  });
